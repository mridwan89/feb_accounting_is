...
test.describe.serial('Permintaan pembayaran sampai dibayar', () => {
  ...
  test('pemohon membuat permintaan, sistem menolak pengajuan tanpa lampiran, lalu pengajuan berhasil', async ({ browser }) => {
    ...
    await page.getByLabel('Keterangan').fill('Konsumsi pelatihan K3 karyawan');
  ...
  test('kepala departemen melihat permintaan di kotak persetujuan dan menyetujuinya', async ({ browser }) => {
    const { page, tutup } = await sebagai(browser, 'kaumum');
  ...
  test('staf akuntansi membuat BKK dari permintaan yang sudah disetujui dan mengajukannya', async ({ browser }) => {
  ...
  test('BKK diperiksa Kepala Bagian Akuntansi lalu disetujui Manajer Keuangan secara berurutan', async ({ browser }) => {
    ...
    const spv = await sebagai(browser, 'spvakt');
