...
before(async () => {
  await siapkan();
  const nama = ['akt1', 'spvakt', 'mankeu', 'kasir1', 'auditor1', 'staf1'];
...
test('AB-16: setiap jurnal seimbang dan neraca saldo seimbang', async () => {
  ...
  const ns = await k.mankeu.get('/api/laporan/neraca-saldo?sampai=2026-09-30');
...
test('LAP-05: buku pembantu utang sama dengan akun kontrol Utang Usaha dan sisa faktur', async () => {
  const r = await k.spvakt.get('/api/laporan/saldo-utang');
...
test('LAP-03: jurnal pengeluaran kas bersaldo seimbang per kolom', async () => {
  const r = await k.mankeu.get('/api/laporan/jurnal-pengeluaran-kas?dari=2026-08-01&sampai=2026-09-30');
...
test('US-16 / AB-15: periode yang ditutup menolak posting sampai dibuka kembali dengan alasan', async () => {
  assert.equal((await k.akt1.post('/api/periode/tutup', { tahun: 2026, bulan: 8 })).status, 403);
  assert.equal((await k.mankeu.post('/api/periode/tutup', { tahun: 2026, bulan: 8 })).status, 200);
  const jm = await k.akt1.post('/api/jurnal-manual', {
  ...
  await k.akt1.post(`/api/jurnal-manual/${jm.body.id}/ajukan`);
  const setuju = await k.mankeu.post(`/api/persetujuan/JM/${jm.body.id}/setujui`);
  assert.equal(setuju.status, 400);
  assert.match(setuju.body.pesan, /Periode Agustus 2026 sudah ditutup/);
  assert.equal((await k.mankeu.post('/api/periode/buka', { tahun: 2026, bulan: 8 })).status, 400, 'alasan wajib');
  assert.equal((await k.mankeu.post('/api/periode/buka', { tahun: 2026, bulan: 8, alasan: 'Koreksi akrual yang disetujui Direktur' })).status, 200);
  assert.equal((await k.mankeu.post(`/api/persetujuan/JM/${jm.body.id}/setujui`)).status, 200);
...
test('Daftar periksa tutup buku menyebut rekonsiliasi yang belum final', async () => {
  const r = await k.mankeu.get('/api/periode/cek?tahun=2026&bulan=8');
  const bca = r.body.find((b) => b.butir.includes('Bank BCA'));
  const mandiri = r.body.find((b) => b.butir.includes('Bank Mandiri'));
  assert.equal(bca.status, 'OK');
  assert.equal(mandiri.status, 'PERHATIAN');
});

test('KF-AKT-02: jurnal manual yang tidak seimbang ditolak; saldo awal hanya untuk Kepala Bagian Akuntansi', async () => {
  const tak = await k.akt1.post('/api/jurnal-manual', {
  ...
  const induk = await k.akt1.post('/api/jurnal-manual', {
  ...
  const utangTanpaPemasok = await k.akt1.post('/api/jurnal-manual', {
  ...
  const saldoAwal = await k.akt1.post('/api/jurnal-manual', {
...
test('KF-AKT-02: tidak ada jalan mengubah atau menghapus jurnal terposting', async () => {
  ...
  assert.equal((await k.spvakt.put(`/api/jurnal/${j.id}`, {})).status, 404);
  assert.equal((await k.spvakt.delete(`/api/jurnal/${j.id}`)).status, 404);
});

test('US-15 / AB-27: rekonsiliasi tidak dapat difinalkan bila selisih, dan tidak boleh dikerjakan Kasir', async () => {
  assert.equal((await k.kasir1.post('/api/rekonsiliasi', { rekening_kas_id: await idRekening('BCA-OPS'), tahun: 2026, bulan: 9, saldo_rekening_koran: 0 })).status, 403);
  const rb = await k.spvakt.post('/api/rekonsiliasi', { rekening_kas_id: await idRekening('BCA-OPS'), tahun: 2026, bulan: 9, saldo_rekening_koran: 123 });
  assert.equal(rb.status, 201, JSON.stringify(rb.body));
  const detail = await k.spvakt.get(`/api/rekonsiliasi/${rb.body.id}`);
  assert.ok(detail.body.beredar.length > 0, 'warkat beredar terisi otomatis');
  const final = await k.spvakt.post(`/api/rekonsiliasi/${rb.body.id}/final`);
...
test('Tanggal kliring pada periode rekonsiliasi yang sudah final tidak dapat diubah', async () => {
  ...
  const r = await k.spvakt.post(`/api/pembayaran/${p.id}/kliring`, { tanggal_kliring: null });
...
test('LAP-06 dan LAP-07: umur utang dan faktur jatuh tempo', async () => {
  const umur = await k.mankeu.get('/api/laporan/umur-utang');
  assert.equal(umur.status, 200);
  assert.ok(umur.body.total_semua > 0);
  const tpj = umur.body.data.find((d) => d.pemasok_nama === 'PT Tinta dan Plastik Jaya');
  ...
  const jt = await k.mankeu.get('/api/laporan/faktur-jatuh-tempo?hari=7');
