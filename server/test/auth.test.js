...
test('KF-ADM-01: masuk berhasil mengembalikan token, peran, dan unit kerja', async () => {
  const r = await request(app).post('/api/auth/masuk').send({ username: 'stafkeu1', password: 'Demo2026' });
  assert.equal(r.status, 200);
  assert.ok(r.body.token.length >= 40);
  assert.deepEqual(r.body.pengguna.peran, ['PEMOHON', 'STAF_KEUANGAN']);
...
test('KF-ADM-01: lima kali salah kata sandi mengunci akun sampai dibuka Administrator', async () => {
  for (let i = 1; i <= 4; i += 1) {
    const r = await request(app).post('/api/auth/masuk').send({ username: 'katu', password: 'salah123' });
    assert.equal(r.status, 401);
    assert.equal(r.body.pesan, 'Nama pengguna atau kata sandi salah.');
  }
  const kelima = await request(app).post('/api/auth/masuk').send({ username: 'katu', password: 'salah123' });
  assert.equal(kelima.status, 401);
  assert.match(kelima.body.pesan, /dikunci 15 menit/);
  const benar = await request(app).post('/api/auth/masuk').send({ username: 'katu', password: 'Demo2026' });
  assert.equal(benar.status, 401);
  assert.match(benar.body.pesan, /terkunci sampai/);

  const admin = await sebagai('admin');
  const buka = await admin.post(`/api/pengguna/${await idPengguna('katu')}/buka-kunci`);
  assert.equal(buka.status, 200);
  assert.ok(await masuk('katu'));
  const log = await satu(pool, "SELECT COUNT(*) AS n FROM log_audit WHERE aksi = 'KUNCI_AKUN' AND entitas_id = ?", [String(await idPengguna('katu'))]);
...
test('KF-ADM-02: akun baru wajib ganti kata sandi dan kebijakan kata sandi ditegakkan', async () => {
  ...
  const buat = await admin.post('/api/pengguna', {
    username: 'ujibaru', nama_lengkap: 'Pengguna Uji', departemen_id: await idDept('AKT'), peran: ['PEMOHON'], password_awal: 'Awal12345',
  });
...
test('Keluar mencabut sesi sehingga token tidak dapat dipakai lagi', async () => {
  const token = await masuk('dosen2');
...
test('KF-ADM-08: masuk dan gagal masuk tercatat di log audit', async () => {
  ...
  const berhasil = await satu(pool, "SELECT COUNT(*) AS n FROM log_audit WHERE aksi = 'MASUK' AND username = 'stafkeu1'");
...
test('KNF-06: percobaan masuk gagal berulang dari satu komputer dibatasi, masuk yang berhasil tidak dihitung', async () => {
  ...
  for (let i = 0; i < 40; i += 1) assert.ok(await masuk('dosen2'));
  ...
  const ditahan = await request(app).post('/api/auth/masuk').send({ username: 'dosen2', password: 'Demo2026' });
