...
before(async () => {
  await siapkan();
  [akt, spv, mankeu, dirkeu, kasir, beli] = await Promise.all(['akt1', 'spvakt', 'mankeu', 'dirkeu', 'kasir1', 'beli1'].map((u) => sebagai(u)));
});
...
test('US-04: BKK tepat Rp50.000.000 tidak butuh Direktur; Rp50.000.001 butuh Direktur', async () => {
  const mk = mankeu;
  const dana = await mk.post('/api/dana-kas-kecil', {
    kode: 'KK-UJI50', nama: 'Kas Kecil Uji Batas', pemegang_id: await id('pengguna', 'username', 'kaskecil2'), departemen_id: await idDept('GDG'),
    akun_id: await idAkun('1-1104'), dana_diusulkan: 60000000, batas_transaksi: 1000000,
  });
  assert.equal(dana.status, 201, JSON.stringify(dana.body));
  const bkk1 = await akt.post('/api/bkk', { jenis: 'PEMBENTUKAN_KAS_KECIL', sumber_id: dana.body.id, jumlah: 50000000, rekening_kas_id: await idRekening('BCA-OPS'), metode_bayar: 'CEK' });
  assert.equal(bkk1.status, 201, JSON.stringify(bkk1.body));
  await akt.post(`/api/bkk/${bkk1.body.id}/ajukan`);
  assert.deepEqual(await langkahBKK(bkk1.body.id), ['SPV_AKUNTANSI', 'MANAJER_KEUANGAN']);
  assert.equal((await akt.post(`/api/bkk/${bkk1.body.id}/batal`, { alasan: 'Uji batas' })).status, 200);

  const bkk2 = await akt.post('/api/bkk', { jenis: 'PEMBENTUKAN_KAS_KECIL', sumber_id: dana.body.id, jumlah: 50000000.01, rekening_kas_id: await idRekening('BCA-OPS'), metode_bayar: 'CEK' });
  assert.equal(bkk2.status, 201, JSON.stringify(bkk2.body));
  await akt.post(`/api/bkk/${bkk2.body.id}/ajukan`);
  assert.deepEqual(await langkahBKK(bkk2.body.id), ['SPV_AKUNTANSI', 'MANAJER_KEUANGAN', 'DIREKTUR']);
  await setujui('BKK', bkk2.body.id, spv, mankeu);
  assert.ok((await dirkeu.get('/api/persetujuan/tugas')).body.some((t) => t.dokumen_id === bkk2.body.id && t.jenis_dokumen === 'BKK'));
});
...
test('US-05 dan US-07: pembayaran cek, lembar terpakai, lalu dibatalkan Manajer Keuangan dengan jurnal pembalik', async () => {
  const faktur = await id('faktur_pemasok', 'nomor_faktur', 'TPJ/07/0098');
  const buat = await akt.post('/api/bkk', {
    jenis: 'PEMBAYARAN_FAKTUR', pemasok_id: await idPemasok('TPJ'), rekening_kas_id: await idRekening('BCA-OPS'), metode_bayar: 'CEK',
  ...
  await setujui('BKK', buat.body.id, spv, mankeu);
  assert.ok((await kasir.get('/api/pembayaran/antrean')).body.some((b) => b.id === buat.body.id));

  const lembar = (await kasir.get(`/api/warkat/tersedia?rekening_kas_id=${await idRekening('BCA-OPS')}&jenis=CEK`)).body;
  ...
  const bkkLain = await akt.post('/api/bkk', { jenis: 'PEMBAYARAN_FAKTUR', pemasok_id: await idPemasok('TPJ'), rekening_kas_id: await idRekening('BCA-OPS'), metode_bayar: 'CEK', faktur: [{ faktur_id: faktur, jumlah: 1 }] });
  ...
  const batal = await mankeu.post(`/api/pembayaran/${bayar.body.id}/batal`, { alasan: 'Cek rusak saat ditulis' });
...
test('AB-11: rekening pemasok yang diubah setelah BKK disetujui memblokir pembayaran transfer', async () => {
  const bkk = await id('bukti_kas_keluar', 'nomor', 'BKK/2026/09/0002');
  await setujui('BKK', bkk, spv, mankeu);
...
test('AB-09: jumlah bayar faktur tidak boleh melebihi sisa utang', async () => {
  ...
  const r = await akt.post('/api/bkk', { jenis: 'PEMBAYARAN_FAKTUR', pemasok_id: await idPemasok('LCP'), rekening_kas_id: await idRekening('BCA-OPS'), metode_bayar: 'CEK', faktur: [{ faktur_id: faktur, jumlah: 5000000 }] });
...
test('KF-BKK-04: satu permintaan pembayaran tidak dapat diproses di dua BKK', async () => {
  ...
  const pp = await staf1.post('/api/pp', { penerima_nama: 'Toko Uji', keterangan: 'Uji BKK ganda', baris: [{ uraian: 'Biaya', akun_id: await idAkun('6-1199'), jumlah: 500000 }] });
  await staf1.unggah(`/api/lampiran/PP/${pp.body.id}`, 'nota.pdf', pdfContoh('Nota'));
  await staf1.post(`/api/pp/${pp.body.id}/ajukan`);
  await setujui('PP', pp.body.id, kaumum);
  const bkk1 = await akt.post('/api/bkk', { jenis: 'PERMINTAAN_PEMBAYARAN', sumber_id: pp.body.id, rekening_kas_id: await idRekening('BCA-OPS'), metode_bayar: 'CEK' });
  assert.equal(bkk1.status, 201);
  const bkk2 = await akt.post('/api/bkk', { jenis: 'PERMINTAAN_PEMBAYARAN', sumber_id: pp.body.id, rekening_kas_id: await idRekening('BCA-OPS'), metode_bayar: 'CEK' });
  ...
  assert.equal((await staf1.get(`/api/pp/${pp.body.id}`)).body.status, 'DISETUJUI');
});

test('KF-BKK-03: potongan PPh atas PP mengurangi jumlah dibayar tanpa mengubah total yang disetujui', async () => {
  const bkk = await satu(pool, "SELECT jumlah_bruto, jumlah_potongan, jumlah_bayar FROM bukti_kas_keluar WHERE sumber_nomor = (SELECT nomor FROM permintaan_pembayaran WHERE keterangan = 'Jasa pendampingan pemeriksaan pajak')");
...
test('AB-10: pembuat BKK tidak dapat mencatat pembayarannya meski lolos pemeriksaan peran', async () => {
  ...
  const ctx = { user: await muatPengguna(pool, await id('pengguna', 'username', 'akt1')), ip: '127.0.0.1' };
...
test('KNF-06: dua pembayaran serentak atas BKK yang sama hanya berhasil satu', async () => {
  const staf1 = await sebagai('staf1');
  const kaumum = await sebagai('kaumum');
  const pp = await staf1.post('/api/pp', { penerima_nama: 'Toko Serentak', penerima_bank_nama: 'BCA', penerima_bank_rekening: '1112223334', penerima_bank_atas_nama: 'Toko Serentak', keterangan: 'Uji serentak', baris: [{ uraian: 'Biaya', akun_id: await idAkun('6-1199'), jumlah: 750000 }] });
  await staf1.unggah(`/api/lampiran/PP/${pp.body.id}`, 'nota.pdf', pdfContoh('Nota'));
  await staf1.post(`/api/pp/${pp.body.id}/ajukan`);
  await setujui('PP', pp.body.id, kaumum);
  const bkk = await akt.post('/api/bkk', { jenis: 'PERMINTAAN_PEMBAYARAN', sumber_id: pp.body.id, rekening_kas_id: await idRekening('BCA-OPS'), metode_bayar: 'TRANSFER' });
  await akt.post(`/api/bkk/${bkk.body.id}/ajukan`);
  await setujui('BKK', bkk.body.id, spv, mankeu);
