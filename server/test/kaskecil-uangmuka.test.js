...
before(async () => {
  await siapkan();
  const nama = ['staf1', 'staf2', 'kaumum', 'kapemasaran', 'kaskecil1', 'kaskecil2', 'akt1', 'spvakt', 'mankeu', 'kasir1', 'auditor1', 'kaprod'];
...
async function bkkDibayar(input, metode = 'CEK') {
  const bkk = await k.akt1.post('/api/bkk', { rekening_kas_id: await idRekening('BCA-OPS'), metode_bayar: metode, ...input });
  assert.equal(bkk.status, 201, JSON.stringify(bkk.body));
  assert.equal((await k.akt1.post(`/api/bkk/${bkk.body.id}/ajukan`)).status, 200);
  await setujui('BKK', bkk.body.id, k.spvakt, k.mankeu);
  const bayar = { bkk_id: bkk.body.id };
  if (metode === 'TRANSFER') bayar.nomor_referensi = `IB-${bkk.body.id}`;
  else bayar.warkat_id = (await k.kasir1.get(`/api/warkat/tersedia?rekening_kas_id=${await idRekening('BCA-OPS')}&jenis=CEK`)).body[0].id;
...
test('US-12: pengeluaran kas kecil di atas batas per transaksi ditolak', async () => {
  const r = await k.staf1.post('/api/pkk', { dana_id: await id('dana_kas_kecil', 'kode', 'KK-PUSAT'), keperluan: 'Printer kecil', akun_id: await idAkun('6-1104'), jumlah: 1200000 });
...
test('US-12 dan US-13: saldo tunai tidak boleh minus; pengisian kembali memulihkan saldo', async () => {
  const dana = await k.mankeu.post('/api/dana-kas-kecil', {
    kode: 'KK-UJI', nama: 'Kas Kecil Uji', pemegang_id: await id('pengguna', 'username', 'kaskecil2'), departemen_id: await idDept('PRD'),
  ...
  const belumDibentuk = await k.staf1.post('/api/pkk', { dana_id: dana.body.id, keperluan: 'Uji', akun_id: await idAkun('6-1104'), jumlah: 10000 });
  ...
  const buatDanDisetujui = async (jumlah) => {
    const pkk = await k.kaprod.post('/api/pkk', { dana_id: dana.body.id, keperluan: `Kebutuhan pabrik ${jumlah}`, akun_id: await idAkun('5-1103'), jumlah });
    assert.equal(pkk.status, 201, JSON.stringify(pkk.body));
    await k.kaprod.post(`/api/pkk/${pkk.body.id}/ajukan`);
    await setujui('PKK', pkk.body.id, k.mankeu); // pembuat adalah Kepala Departemen: dialihkan ke Manajer Keuangan
    return pkk.body.id;
  };
  ...
  const detailBkk = await k.akt1.get(`/api/bkk/${bkk.id}`);
...
test('US-14: opname menghitung selisih dan terkunci setelah final; pemegang dana tidak dapat mengopname', async () => {
  const dana = await id('dana_kas_kecil', 'kode', 'KK-PABRIK');
...
test('US-10: uang muka yang lewat tenggat memblokir pengajuan uang muka baru', async () => {
  const lewat = await satu(pool, "SELECT nomor, tanggal_batas_pj FROM uang_muka WHERE keperluan = 'Kunjungan pelanggan di Cirebon'");
  assert.ok(lewat.tanggal_batas_pj < '2026-09-25');
  const baru = await k.staf2.post('/api/uang-muka', { keperluan: 'Pameran Surabaya', jumlah: 2500000, tanggal_selesai_kegiatan: '2026-10-05' });
  assert.equal(baru.status, 201);
  const aju = await k.staf2.post(`/api/uang-muka/${baru.body.id}/ajukan`);
  assert.equal(aju.status, 409);
  assert.match(aju.body.pesan, new RegExp(lewat.nomor));
  const lap = await k.mankeu.get('/api/laporan/uang-muka-beredar');
...
test('US-11: pertanggungjawaban dengan sisa diselesaikan lewat BKM sebesar sisa tepat', async () => {
  const um = await satu(pool, "SELECT id, nomor FROM uang_muka WHERE keperluan = 'Kunjungan pelanggan di Cirebon'");
  const pj = await k.staf2.post('/api/pjum', {
    uang_muka_id: um.id,
    baris: [
  ...
  const tanpaLampiran = await k.staf2.post(`/api/pjum/${pj.body.id}/ajukan`);
  assert.equal(tanpaLampiran.status, 400);
  assert.match(tanpaLampiran.body.pesan, /belum punya lampiran/);
  await k.staf2.unggah(`/api/lampiran/PJUM/${pj.body.id}`, 'bukti.pdf', pdfContoh('Bukti perjalanan'));
  assert.equal((await k.staf2.post(`/api/pjum/${pj.body.id}/ajukan`)).status, 200);
  await setujui('PJUM', pj.body.id, k.kapemasaran, k.spvakt);
  const detail = await k.staf2.get(`/api/pjum/${pj.body.id}`);
  ...
  const salah = await k.kasir1.post('/api/bkm', { rekening_kas_id: await idRekening('BCA-OPS'), sumber: 'PENGEMBALIAN_UANG_MUKA', sumber_id: pj.body.id, jumlah: 250000 });
  assert.equal(salah.status, 400);
  const bkm = await k.kasir1.post('/api/bkm', { rekening_kas_id: await idRekening('BCA-OPS'), sumber: 'PENGEMBALIAN_UANG_MUKA', sumber_id: pj.body.id, jumlah: 300000 });
  ...
  const baru = await satu(pool, "SELECT id FROM uang_muka WHERE keperluan = 'Pameran Surabaya'");
  assert.equal((await k.staf2.post(`/api/uang-muka/${baru.id}/ajukan`)).status, 200);
});

test('US-11: kekurangan uang muka dicatat sebagai utang kepada karyawan lalu dibayar melalui BKK', async () => {
