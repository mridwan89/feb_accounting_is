...
before(async () => {
  await siapkan();
  const nama = ['dosen1', 'dosen2', 'kaprodiakt', 'pengadaan1', 'kasubag', 'auditor1', 'stafkeu1'];
...
test('KF-DOK-01: lampiran diperiksa dari isinya; berkas yang disamarkan ditolak', async () => {
  const pp = await k.dosen1.post('/api/pp', { penerima_nama: 'Toko Lampiran', keterangan: 'Uji lampiran', baris: [{ uraian: 'Biaya', akun_id: await idAkun('6-1199'), jumlah: 100000 }] });
  const ok = await k.dosen1.unggah(`/api/lampiran/PP/${pp.body.id}`, 'nota.pdf', pdfContoh('Nota asli'));
  assert.equal(ok.status, 201, JSON.stringify(ok.body));
  assert.equal(ok.body.tipe_mime, 'application/pdf');
  const palsu = await k.dosen1.unggah(`/api/lampiran/PP/${pp.body.id}`, 'nota.pdf', Buffer.from('MZ\x90\x00 ini sebenarnya program'));
  assert.equal(palsu.status, 400);
  assert.match(palsu.body.pesan, /Unggah PDF, JPG, atau PNG/);
  const png = await k.dosen1.unggah(`/api/lampiran/PP/${pp.body.id}`, 'foto.png', Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0, 0, 0, 0]));
  assert.equal(png.status, 201);
  assert.equal((await k.dosen1.get(`/api/lampiran/PP/${pp.body.id}`)).body.length, 2);

  // Sebelum diajukan pengunggah boleh menghapus; sesudah diajukan tidak boleh.
  assert.equal((await k.dosen1.delete(`/api/lampiran-berkas/${png.body.id}`)).status, 200);
  await k.dosen1.post(`/api/pp/${pp.body.id}/ajukan`);
  const hapus = await k.dosen1.delete(`/api/lampiran-berkas/${ok.body.id}`);
  assert.equal(hapus.status, 409);

  // Unduh hanya oleh yang berhak melihat dokumen.
  assert.equal((await k.dosen2.get(`/api/lampiran-berkas/${ok.body.id}`)).status, 403);
  const unduh = await k.kaprodiakt.get(`/api/lampiran-berkas/${ok.body.id}`);
...
test('KF-DOK-04: cetak pertama tercatat asli, cetak berikutnya salinan', async () => {
  const pp = await satu(pool, "SELECT id FROM permintaan_pembayaran WHERE keterangan = 'Tagihan listrik gedung FEB bulan lalu'");
  const c1 = await k.stafkeu1.post(`/api/cetak/PP/${pp.id}`);
  const c2 = await k.stafkeu1.post(`/api/cetak/PP/${pp.id}`);
  ...
  assert.equal((await k.dosen2.post(`/api/cetak/PP/${pp.id}`)).status, 403);
});
...
test('US-18 / AB-26: perubahan rekening pemasok tercatat lengkap dan diverifikasi pihak lain', async () => {
  ...
  const lama = (await k.pengadaan1.get(`/api/pemasok/${idTma}`)).body;
  const ubah = await k.pengadaan1.put(`/api/pemasok/${idTma}`, { ...lama, bank_nomor_rekening: '0098877000' });
  ...
  assert.equal(log.username, 'pengadaan1');
  assert.ok(log.ip);
  ...
  assert.equal((await k.kasubag.post(`/api/pemasok/${idTma}/verifikasi-rekening`)).status, 200);
  assert.equal((await k.kasubag.post(`/api/pemasok/${idTma}/verifikasi-rekening`)).status, 409);
...
test('KF-MST-09: NPWP dan PKP divalidasi pada data pemasok', async () => {
  const salah = await k.pengadaan1.post('/api/pemasok', { kode: 'UJI-NPWP', nama: 'CV Uji', termin_hari: 30, npwp: '12345', pkp: true });
  assert.equal(salah.status, 400);
  assert.equal(salah.body.galat.npwp, 'NPWP harus 15 atau 16 digit.');
  const pkpTanpaNpwp = await k.pengadaan1.post('/api/pemasok', { kode: 'UJI-PKP', nama: 'CV Uji PKP', termin_hari: 30, pkp: true });
  assert.equal(pkpTanpaNpwp.status, 400);
  const rekeningSetengah = await k.pengadaan1.post('/api/pemasok', { kode: 'UJI-BANK', nama: 'CV Uji Bank', termin_hari: 30, bank_nama: 'BCA' });
